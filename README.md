...
89 tests · 2,540 lines of Rust · 9 modules
...
```rust
// Agent population with coupling strength
let pop = AgentPopulation {
    name: "Cooperative".into(),
    coupling: 0.5,
    size: 1000,
    topology: InteractionTopology::Lattice { dimension: 2 },
};

// RG flow for agent behavior across organizational scales
let flow = agent_rg_flow(0.5, &beta_fn, 100, 1.1);

// Classify behavioral regime
let regime = classify_agent_behavior(0.5, 1.0); // Disordered
let regime = classify_agent_behavior(1.0, 1.0); // Critical
let regime = classify_agent_behavior(2.0, 1.0); // Ordered

// Effective coupling at organizational scale
let g_eff = effective_coupling_at_scale(1.0, 10.0, &beta_fn);

// Detect phase transitions in agent data
let kc = detect_phase_transition(&order_params, &couplings);

// Agent universality classes
...
Layer 6: Scaling              scaling (ξ, M, χ, C, finite-size, corrections)
              │
              ▼
Layer 7: Agent Application    agent (populations, phase transitions, behavioral universality)
```

**Layer 1** defines the beta function β(g) = dg/d(ln μ) and integrates it numerically. Couplings are classified as relevant (grows under RG), irrelevant (decays), or marginal (need higher-order analysis).

**Layer 2** finds fixed points where β(g*)=0 using Newton's method and scanning. Each fixed point is classified as stable (IR-attractive) or unstable (UV-attractive) by the eigenvalue β'(g*).

**Layer 3** computes critical exponents from fixed point eigenvalues. The exponents satisfy four scaling relations (Rushbrooke, Widom, Fisher, Josephson hyperscaling) — only two are independent in d dimensions.

**Layer 4** provides three approaches: the epsilon expansion (perturbation theory in d=4−ε), Wilsonian RG (integrate out momentum shells), and real-space RG (coarse-grain in position space).

**Layer 5** catalogs universality classes — groups of systems sharing the same fixed point and critical exponents.

**Layer 6** implements scaling forms for physical observables and finite-size scaling for numerical analysis.

**Layer 7** applies the RG framework to agent populations, treating collective behavior as a statistical mechanics problem.
...
| `agent` | 10 | `AgentPopulation`, `AgentRGFlow`, `BehavioralRegime` | Agent behavior across scales |
